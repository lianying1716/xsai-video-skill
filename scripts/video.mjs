...
const CONSUMER_CLIENT_ID = "xsai-video-skill";
const BASE_URL = process.env.XSAI_VIDEO_BASE_URL || "https://api.xsai5.xyz";
const AUTH_BASE_URL = process.env.XSAI_VIDEO_AUTH_BASE_URL || "https://xsai5.xyz";
...
const { readState, removeState, tokenFromRefresh, apiRequest, authorizedRequest, downloadFile, login } = runtime;
...
function normalizeVideoError(error) {
  ...
  const known = new Set(["auth_required", "auth_recovery_required", "auth_busy", "authorization_pending", "invalid_request", "invalid_scope", "spend_confirmation_required", "unsupported_option", "model_unavailable", "not_found", "timeout"]);
  const result = { code, message: known.has(code) ? String(error.message || "请求失败") : "视频服务暂时不可用，请稍后重试。" };
  if (error?.jobId) result.job_id = String(error.jobId);
  if (error?.resumeCommand) result.resume_command = String(error.resumeCommand);
  return result;
}
...
async function watchStatuses({ jobId, fetchStatus, timeoutMs = 900_000, initialDelayMs = 30_000, intervalMs = 8_000, maxPolls = 120, maxRetries = 2, sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms)), now = Date.now, onProgress = () => {} } = {}) {
  const started = now(); let polls = 0; let lastStatus = null;
  const remaining = () => Math.max(0, timeoutMs - (now() - started));
  if (initialDelayMs > 0) {
    const waitMs = Math.min(initialDelayMs, remaining());
    onProgress({ job_id: jobId, status: "waiting", next_poll_seconds: Math.round(waitMs / 1000), polls });
    if (waitMs < initialDelayMs) throw Object.assign(new Error("视频任务轮询超时"), { code: "timeout", jobId, resumeCommand: jobId ? `node scripts/video.mjs watch ${jobId}` : undefined });
    await sleep(waitMs);
  }
  while (remaining() > 0 && polls < maxPolls) {
    let status;
    for (let retry = 0; retry <= maxRetries; retry += 1) {
      try {
        ...
        const transient = Number(error?.status) >= 500 || ["fetch_failed", "network_error", "timeout", "http_408", "http_429", "http_502", "http_503", "http_504"].includes(String(error?.code || ""));
        if (!transient || retry === maxRetries) {
          if (transient) Object.assign(error, { jobId, resumeCommand: jobId ? `node scripts/video.mjs watch ${jobId}` : undefined });
          throw error;
        }
        const retryAfter = Number(error?.retryAfterMs || error?.retry_after_ms || 0);
        const retryWait = Math.min(60_000, Math.max(1_000, retryAfter || 1_000 * (2 ** retry)));
        if (remaining() < retryWait) throw Object.assign(new Error("视频任务轮询超时"), { code: "timeout", jobId, lastStatus, resumeCommand: jobId ? `node scripts/video.mjs watch ${jobId}` : undefined });
        await sleep(retryWait);
      }
    }
    polls += 1;
    lastStatus = status;
    onProgress({ job_id: jobId, status: status?.status || "processing", polls, next_poll_seconds: Math.round((Number(status?.poll_after_seconds || status?.retry_after_seconds || status?.next_poll_after_seconds) || intervalMs / 1000)) });
    if (["succeeded", "failed", "cancelled", "timeout", "submission_unknown"].includes(String(status?.status || "").toLowerCase())) return status;
    const suggested = Number(status?.poll_after_seconds || status?.retry_after_seconds || status?.next_poll_after_seconds);
    const nextDelay = Number.isFinite(suggested) && suggested > 0 ? Math.min(60_000, Math.max(2_000, suggested * 1000)) : intervalMs;
    if (remaining() <= 0) break;
    await sleep(Math.min(nextDelay, remaining()));
  }
  throw Object.assign(new Error("视频任务轮询超时"), { code: "timeout", jobId, lastStatus, resumeCommand: jobId ? `node scripts/video.mjs watch ${jobId}` : undefined });
}
async function downloadJob(jobId, output, fetchImpl = globalThis.fetch) {
  ...
  const target = await downloadFile(item.url, path.resolve(output || `video-${jobId}.mp4`), { fetchImpl });
  return { job_id: jobId, output: target };
}
async function main(argv = process.argv.slice(2), fetchImpl = globalThis.fetch, watchOptions = {}) {
  ...
  if (args.command === "auth" && args.subcommand === "status") { const state = await readState(); const scopes = Array.isArray(state?.scopes) ? state.scopes : String(state?.scopes || "").split(/\s+/).filter(Boolean); return state ? { status: "configured", scopes, base_url: state.base_url || BASE_URL } : { status: "signed_out" }; }
  ...
  if (args.command === "create") {
    const request = buildVideoRequest(args, { profiles }); delete request._meta;
    Object.assign(request, await prepareVideoReferences(args, token, fetchImpl));
    const capability = await apiRequest(`/v1/videos/capabilities?model=${encodeURIComponent(request.model)}`, { token, fetchImpl });
    if (!Array.isArray(capability?.data) || !capability.data.some((item) => String(item?.id || "") === request.model && item.available !== false)) throw Object.assign(new Error("当前授权下模型不可用"), { code: "model_unavailable" });
    const created = await apiRequest("/v1/videos", { method: "POST", token, body: request, fetchImpl });
    if (!args.wait) return created;
    const jobId = created?.id || created?.job_id || created?.request_id;
    if (!jobId) throw Object.assign(new Error("视频任务缺少任务 ID，无法等待"), { code: "submission_unknown" });
    const watched = await watchStatuses({ ...watchOptions, jobId, timeoutMs: Math.max(1, Number(args.timeout || 900)) * 1000, initialDelayMs: Math.max(0, Number(args["initial-delay"] ?? 30)) * 1000, fetchStatus: () => authorizedRequest(`/v1/videos/${encodeURIComponent(jobId)}`, { fetchImpl }) });
    return { ...watched, job_id: jobId };
  }
  if (args.command === "watch") { const jobId = args._[1]; return watchStatuses({ ...watchOptions, jobId, timeoutMs: Math.max(1, Number(args.timeout || 900)) * 1000, initialDelayMs: Math.max(0, Number(args["initial-delay"] ?? 30)) * 1000, fetchStatus: () => authorizedRequest(`/v1/videos/${encodeURIComponent(jobId)}`, { fetchImpl }) }); }
